...
### Fixed
- Migration `refs.0002` referenced `'omniman_refs'` instead of `'refs'`, causing `NodeNotFoundError` when `omniman.contrib.refs` was included in `INSTALLED_APPS`.
- Admin fallback `ChoicesRadioFilter` inherited from `SimpleListFilter` instead of `FieldListFilter`, triggering `admin.E115` system check errors on Django 5.x without Unfold installed.
