...
from omniman.models import Channel, Order, OrderEvent, OrderItem, Session
...
class OrderEventAuditTests(TestCase):
    ...
    def setUp(self) -> None:
        self.channel = Channel.objects.create(code="test", name="Test")
        self.order = Order.objects.create(
            ref="AUDIT-001",
            channel=self.channel,
            status=Order.STATUS_NEW,
...
class SessionToOrderFlowTests(TestCase):
    ...
    def setUp(self) -> None:
        self.channel = Channel.objects.create(code="flow", name="Flow Test")
