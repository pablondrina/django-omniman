...
class ProcessDirectivesCommandTests(TestCase):
    ...
    def test_process_directives_runs_registered_handler(self) -> None:
        directive = Directive.objects.create(topic=self.handler.topic, payload={}, status="queued")
    ...
    def test_process_directives_handles_unknown_topic(self) -> None:
        ...
        Directive.objects.create(topic="unknown", payload={}, status="queued")
    ...
    def test_process_directives_with_limit(self) -> None:
        ...
        Directive.objects.create(topic=self.handler.topic, payload={}, status="queued")
        Directive.objects.create(topic=self.handler.topic, payload={}, status="queued")
        Directive.objects.create(topic=self.handler.topic, payload={}, status="queued")
    ...
    def test_process_directives_with_specific_topic(self) -> None:
        ...
        Directive.objects.create(topic=self.handler.topic, payload={}, status="queued")
        Directive.objects.create(topic="other.topic", payload={}, status="queued")
