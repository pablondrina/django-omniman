...
from omniman.exceptions import CommitError, SessionError
from omniman.models import Channel, IdempotencyKey, Order, Session
...
class CommitIdempotencyTests(TestCase):
    ...
    def test_commit_in_progress_raises_error(self) -> None:
        ...
        IdempotencyKey.objects.create(
            scope=f"commit:{self.channel.code}",
            key="in-progress-key",
            status="in_progress",
        )
