...
from omniman.models import Channel, Directive, IdempotencyKey, Session
...
class SessionApiTests(TestCase):
    ...
    def test_create_session_basic(self) -> None:
        self._mk_channel(code="pos")
        resp = self.client.post("/api/sessions", {"channel_code": "pos"}, format="json")
        self.assertEqual(resp.status_code, 201, resp.data)
        self.assertEqual(resp.data["channel_code"], "pos")
    ...
    def test_create_session_get_or_open_by_owner(self) -> None:
        self._mk_channel(code="pos")
        payload = {"channel_code": "pos", "handle_type": "mesa", "handle_ref": "T12"}
    ...
    def test_retrieve_requires_channel_code_if_ambiguous(self) -> None:
        c1 = self._mk_channel(code="pos")
        c2 = self._mk_channel(code="shop")
        ...
        self.assertIn("channel_code", r.data)

        r2 = self.client.get("/api/sessions/SESS-AMBIG?channel_code=pos")
        self.assertEqual(r2.status_code, 200, r2.data)
        self.assertEqual(r2.data["channel_code"], "pos")
    ...
    def test_modify_increments_rev_clears_checks_issues_and_enqueues_directive(self) -> None:
        ...
        s = self.client.post("/api/sessions", {"channel_code": "pos"}, format="json").data

        payload = {"channel_code": "pos", "ops": [{"op": "add_line", "sku": "LATTE", "qty": 2}]}
    ...
    def test_modify_validates_qty_positive(self) -> None:
        ...
        r = self.client.post(
            f"/api/sessions/{s['session_key']}/modify",
            {"channel_code": "pos", "ops": [{"op": "add_line", "sku": "LATTE", "qty": 0}]},
            format="json",
        )
        self.assertEqual(r.status_code, 400, r.data)
    ...
    def test_modify_external_pricing_requires_unit_price_q(self) -> None:
        self._mk_channel(code="ifood", pricing_policy="external")
        s = self.client.post("/api/sessions", {"channel_code": "ifood"}, format="json").data

        r = self.client.post(
            f"/api/sessions/{s['session_key']}/modify",
            {"channel_code": "ifood", "ops": [{"op": "add_line", "sku": "LATTE", "qty": 1}]},
            format="json",
        )
        self.assertEqual(r.status_code, 400, r.data)
    ...
    def test_commit_requires_fresh_required_checks(self) -> None:
        c = self._mk_channel(code="pos", config={"required_checks_on_commit": ["stock"]})
        ...
        r1 = self.client.post(f"/api/sessions/{sess.session_key}/commit", {"channel_code": "pos"}, format="json")
        ...
        r2 = self.client.post(f"/api/sessions/{sess.session_key}/commit", {"channel_code": "pos"}, format="json")
    ...
    def test_commit_blocks_on_blocking_issues(self) -> None:
        ...
        r = self.client.post(f"/api/sessions/{sess.session_key}/commit", {"channel_code": "pos"}, format="json")
        self.assertEqual(r.status_code, 400, r.data)
    ...
    def test_commit_is_idempotent(self) -> None:
        ...
        r1 = self.client.post(
            ...
            {"channel_code": "pos", "idempotency_key": idem},
            format="json",
        )
        self.assertEqual(r1.status_code, 201, r1.data)
    ...
    def test_resolve_delegates_by_source_and_applies_ops(self) -> None:
        ...
        r = self.client.post(
            ...
            {"channel_code": "pos", "issue_id": issue_id, "action_id": action_id},
            format="json",
        )
        self.assertEqual(r.status_code, 200, r.data)
        self.assertEqual(r.data["rev"], 1)
