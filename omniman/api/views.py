...
from omniman.exceptions import CommitError, IssueResolveError, SessionError, ValidationError
...
class SessionViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, mixins.CreateModelMixin, viewsets.GenericViewSet):
    ...
    Endpoints:
        ...
        GET  /api/sessions/{key}?channel_code=X - Detalhes da sessão
    ...
    Notas:
        ...
        - Para rotas detail, channel_code pode ser query param ou body
    ...
    def _get_channel_code_from_request(self) -> str | None:
        # Para GET /retrieve, `channel_code` tende a vir em querystring.
        code = self.request.query_params.get("channel_code")
        if code:
            return code
        # Para POST actions, pode vir no body.
        data = getattr(self.request, "data", {}) or {}
        channel = data.get("channel") or {}
        # Quando usamos SlugRelatedField(source="channel") o payload original é channel_code,
        # mas aqui ainda não passou pelo serializer; então lemos direto.
        return data.get("channel_code")
    ...
    def get_object(self):
        ...
        channel_code = self._get_channel_code_from_request()
        ...
        if not matches:
            raise NotFound("Sessão não encontrada.")
        if len(matches) > 1 and not channel_code:
            raise DRFValidationError(
                {"channel_code": "Obrigatório quando session_key existe em mais de um canal."}
            )
        return matches[0]
