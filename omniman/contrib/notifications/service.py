"""
Omniman Notifications Service — Serviço central (simples).
"""
...
def register_backend(name: str, backend: NotificationBackend) -> None:
    """
    Registra um backend de notificação.

    Args:
        name: Nome do backend (ex: "whatsapp", "sms")
        backend: Instância do backend
    """
...
def get_backend(name: str | None = None) -> NotificationBackend | None:
    ...
    Returns:
        Backend ou None se não encontrado
    """
    if name is None:
        config = getattr(settings, "OMNIMAN_NOTIFICATIONS", {})
