"""
Omniman Pricing Modifiers — Modifiers para precificação.
...
class ItemPricingModifier:
    ...
    Responsabilidades:
    1. Define unit_price_q para itens sem preço (usando PricingBackend)
    2. Calcula line_total_q = qty * unit_price_q
    ...
    def apply(self, *, channel: Any, session: Any, ctx: dict) -> None:
        """
        Aplica preços e calcula totais de linha.

        Para cada item:
        1. Se não tem unit_price_q, busca do backend
        2. Calcula line_total_q = qty * unit_price_q
        """
...
class SessionTotalModifier:
    ...
    def apply(self, *, channel: Any, session: Any, ctx: dict) -> None:
        """Calcula total da sessão somando line_total_q de todos os itens."""
