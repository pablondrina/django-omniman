...
class _Registry:
    ...
    def get_validators(self, stage: str | None = None) -> list[Validator]:
        """
        Retorna validators registrados.

        Args:
            stage: Se fornecido, filtra por stage ("draft", "commit", "import")
        """
        with self._lock:
            if stage is None:
    ...
    def get_modifiers(self) -> list[Modifier]:
        """Retorna modifiers registrados, ordenados por `.order`."""
        with self._lock:
    ...
    def register_directive_handler(self, handler: DirectiveHandler) -> None:
        """
        Registra um handler de diretiva.

        Nota: Um topic só pode ter um handler registrado.
        """
    ...
    def register_issue_resolver(self, resolver: IssueResolver) -> None:
        """
        Registra um resolver de issues.

        Nota: Uma source só pode ter um resolver registrado.
        """
    ...
    def get_issue_resolver(self, source: str) -> IssueResolver | None:
        """Retorna resolver para uma source específica."""
        with self._lock:
            return self._issue_resolvers.get(source)

    def get_issue_resolvers(self) -> dict[str, IssueResolver]:
        """Retorna todos os resolvers de issues registrados."""
        with self._lock:
